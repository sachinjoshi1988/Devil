...
class DefaultLearningResultMapperTest {

    @Test
    fun `map translates learnable evaluation into learnable result`() {
        val traceId = TraceId.from(
            "trace-learning-result-mapper-001",
        )
        val request = createRequest(traceId)
        val mapper: LearningResultMapper =
            DefaultLearningResultMapper()

        val result = mapper.map(
            traceId = traceId,
            evaluation = LearningEvaluationResult.create(
                traceId = traceId,
    ...
    fun `map translates unavailable evaluation into operational deferral`() {
        val traceId = TraceId.from(
            "trace-learning-result-mapper-002",
        )

        val result = DefaultLearningResultMapper().map(
            traceId = traceId,
            evaluation = LearningEvaluationResult.create(
                traceId = traceId,
    ...
    fun `map preserves failed evaluation error`() {
        val traceId = TraceId.from(
            "trace-learning-result-mapper-003",
        )
    ...
    @Test
    fun `map does not create memory or mutate task or plan state`() {
        val traceId = TraceId.from(
            "trace-learning-result-mapper-004",
        )
    ...
    fun `map rejects evaluation result from a different trace`() {
        assertFailsWith<IllegalArgumentException> {
            DefaultLearningResultMapper().map(
                traceId = TraceId.from(
                    "trace-learning-result-mapper-005",
                ),
                evaluation = LearningEvaluationResult.create(
                    traceId = TraceId.from(
    ...
    private fun createPlan(
        ...
        return PlanRecord.create(
            planId = PlanId.from(
                "plan-learning-result-mapper",
            ),
            task = TaskRecord.create(
                taskId = TaskId.from(
                    "task-learning-result-mapper",
                ),
