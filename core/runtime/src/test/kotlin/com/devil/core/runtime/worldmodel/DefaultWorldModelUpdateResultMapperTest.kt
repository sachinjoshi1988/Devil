...
class DefaultWorldModelUpdateResultMapperTest {

    @Test
    fun `map translates applicable evaluation into applicable result`() {
        val traceId = TraceId.from(
            "trace-world-model-update-result-mapper-001",
        )
        val request = createRequest(traceId)
        val mapper: WorldModelUpdateResultMapper =
            DefaultWorldModelUpdateResultMapper()

        val result = mapper.map(
            traceId = traceId,
            evaluation = WorldModelUpdateEvaluationResult.create(
                traceId = traceId,
    ...
    fun `map translates unavailable evaluation into operational deferral`() {
        val traceId = TraceId.from(
            "trace-world-model-update-result-mapper-002",
        )

        val result = DefaultWorldModelUpdateResultMapper().map(
            traceId = traceId,
            evaluation = WorldModelUpdateEvaluationResult.create(
                traceId = traceId,
    ...
    fun `map preserves failed evaluation error`() {
        val traceId = TraceId.from(
            "trace-world-model-update-result-mapper-003",
        )
    ...
    @Test
    fun `map does not mutate World Model task or plan state`() {
        val traceId = TraceId.from(
            "trace-world-model-update-result-mapper-004",
        )
        ...
        assertFailsWith<IllegalArgumentException> {
            DefaultWorldModelUpdateResultMapper().map(
                traceId = TraceId.from(
                    "trace-world-model-update-result-mapper-005",
                ),
                evaluation = WorldModelUpdateEvaluationResult.create(
                    traceId = TraceId.from(
    ...
    private fun createPlan(
        ...
        return PlanRecord.create(
            planId = PlanId.from(
                "plan-world-model-update-result-mapper",
            ),
            task = TaskRecord.create(
                taskId = TaskId.from(
                    "task-world-model-update-result-mapper",
                ),
