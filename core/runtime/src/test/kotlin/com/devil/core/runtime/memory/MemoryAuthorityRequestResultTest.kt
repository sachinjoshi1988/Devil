...
class MemoryAuthorityRequestResultTest {

    @Test
    fun `create preserves available result with matching request`() {
        val traceId = TraceId.from(
            "trace-memory-authority-request-result-001",
        )
        val request = createRequest(traceId)

        val result = MemoryAuthorityRequestResult.create(
            traceId = traceId,
    ...
    fun `create preserves unavailable result`() {
        val traceId = TraceId.from(
            "trace-memory-authority-request-result-002",
        )
    ...
    fun `create preserves failed result with matching error`() {
        val traceId = TraceId.from(
            "trace-memory-authority-request-result-003",
        )
        ...
        assertFailsWith<IllegalArgumentException> {
            MemoryAuthorityRequestResult.create(
                traceId = TraceId.from(
                    "trace-memory-authority-request-result-004",
                ),
                status = MemoryAuthorityRequestStatus.AVAILABLE,
            )
        }
    ...
    fun `create rejects request from another trace`() {
        assertFailsWith<IllegalArgumentException> {
            MemoryAuthorityRequestResult.create(
                traceId = TraceId.from(
                    "trace-memory-authority-request-result-005",
                ),
                status = MemoryAuthorityRequestStatus.AVAILABLE,
    ...
    fun `create rejects unavailable result with request`() {
        val traceId = TraceId.from(
            "trace-memory-authority-request-result-006",
        )
    ...
    fun `create rejects unavailable result with error`() {
        val traceId = TraceId.from(
            "trace-memory-authority-request-result-007",
        )
    ...
    fun `create rejects failed result without error`() {
        assertFailsWith<IllegalArgumentException> {
            MemoryAuthorityRequestResult.create(
                traceId = TraceId.from(
                    "trace-memory-authority-request-result-008",
                ),
                status = MemoryAuthorityRequestStatus.FAILED,
            )
        }
    ...
    fun `create rejects failed error from another trace`() {
        assertFailsWith<IllegalArgumentException> {
            MemoryAuthorityRequestResult.create(
                traceId = TraceId.from(
                    "trace-memory-authority-request-result-009",
                ),
                status = MemoryAuthorityRequestStatus.FAILED,
                error = createError(
                    TraceId.from(
    ...
    @Test
    fun `create does not reinterpret proposal dependencies`() {
        val traceId = TraceId.from(
            "trace-memory-authority-request-result-010",
        )
    ...
    private fun createPlan(
        ...
        val task = TaskRecord.create(
            taskId = TaskId.from(
                "task-memory-authority-request-result",
            ),
            decision = decision,
        ...
        return PlanRecord.create(
            planId = PlanId.from(
                "plan-memory-authority-request-result",
            ),
            task = task,
    ...
    private fun createError(
        ...
        return UniversalErrorRecord.create(
            errorCode = ErrorCode.from(
                "MEMORY_AUTHORITY_REQUEST_FAILED",
            ),
            traceId = traceId,
            occurredAt =
                DevilTimestamp.fromEpochMilliseconds(
            ...
            summary =
                "Memory Authority request preparation failed.",
        )
    }
}
