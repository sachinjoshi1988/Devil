...
class MemoryProposalEvaluationResultTest {

    @Test
    fun `create preserves proposable result with matching request`() {
        val traceId = TraceId.from(
            "trace-memory-proposal-evaluation-result-001",
        )
        val request = createRequest(traceId)

        val result = MemoryProposalEvaluationResult.create(
            traceId = traceId,
            status = MemoryProposalEvaluationStatus.PROPOSABLE,
            request = request,
        )
    ...
    fun `create preserves unavailable result`() {
        val traceId = TraceId.from(
            "trace-memory-proposal-evaluation-result-002",
        )
    ...
    fun `create preserves failed result with matching error`() {
        val traceId = TraceId.from(
            "trace-memory-proposal-evaluation-result-003",
        )
        ...
        assertFailsWith<IllegalArgumentException> {
            MemoryProposalEvaluationResult.create(
                traceId = TraceId.from(
                    "trace-memory-proposal-evaluation-result-004",
                ),
                status = MemoryProposalEvaluationStatus.PROPOSABLE,
            )
        }
    ...
    fun `create rejects proposable request from another trace`() {
        assertFailsWith<IllegalArgumentException> {
            MemoryProposalEvaluationResult.create(
                traceId = TraceId.from(
                    "trace-memory-proposal-evaluation-result-005",
                ),
                status = MemoryProposalEvaluationStatus.PROPOSABLE,
                request = createRequest(
                    TraceId.from(
                        "trace-memory-proposal-evaluation-request-other",
                    ),
                ),
            )
        }
    ...
    fun `create rejects unavailable result with request`() {
        val traceId = TraceId.from(
            "trace-memory-proposal-evaluation-result-006",
        )
    ...
    fun `create rejects failed result without error`() {
        assertFailsWith<IllegalArgumentException> {
            MemoryProposalEvaluationResult.create(
                traceId = TraceId.from(
                    "trace-memory-proposal-evaluation-result-007",
                ),
                status = MemoryProposalEvaluationStatus.FAILED,
            )
        }
    ...
    fun `create rejects failed error from another trace`() {
        assertFailsWith<IllegalArgumentException> {
            MemoryProposalEvaluationResult.create(
                traceId = TraceId.from(
                    "trace-memory-proposal-evaluation-result-008",
                ),
                status = MemoryProposalEvaluationStatus.FAILED,
                error = createError(
                    TraceId.from(
                        "trace-memory-proposal-evaluation-error-other",
                    ),
                ),
            )
        }
    ...
    private fun createPlan(
        ...
        return PlanRecord.create(
            planId = PlanId.from(
                "plan-memory-proposal-evaluation-result",
            ),
            task = TaskRecord.create(
                taskId = TaskId.from(
                    "task-memory-proposal-evaluation-result",
                ),
    ...
    private fun createError(
        ...
        return UniversalErrorRecord.create(
            errorCode = ErrorCode.from(
                "MEMORY_PROPOSAL_EVALUATION_FAILED",
            ),
            traceId = traceId,
            occurredAt =
