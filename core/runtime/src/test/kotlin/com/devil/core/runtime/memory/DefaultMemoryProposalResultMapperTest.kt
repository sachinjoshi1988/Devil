...
class DefaultMemoryProposalResultMapperTest {

    @Test
    fun `map translates proposable evaluation into proposable result`() {
        val traceId = TraceId.from(
            "trace-memory-proposal-result-mapper-001",
        )
        val request = createRequest(traceId)
        val mapper: MemoryProposalResultMapper =
            DefaultMemoryProposalResultMapper()

        val result = mapper.map(
            traceId = traceId,
            evaluation = MemoryProposalEvaluationResult.create(
                traceId = traceId,
    ...
    fun `map translates unavailable evaluation into operational deferral`() {
        val traceId = TraceId.from(
            "trace-memory-proposal-result-mapper-002",
        )

        val result = DefaultMemoryProposalResultMapper().map(
            traceId = traceId,
            evaluation = MemoryProposalEvaluationResult.create(
                traceId = traceId,
    ...
    fun `map preserves failed evaluation error`() {
        val traceId = TraceId.from(
            "trace-memory-proposal-result-mapper-003",
        )
    ...
    fun `map does not commit memory or mutate task or plan state`() {
        val traceId = TraceId.from(
            "trace-memory-proposal-result-mapper-004",
        )
    ...
    fun `map rejects evaluation result from another trace`() {
        assertFailsWith<IllegalArgumentException> {
            DefaultMemoryProposalResultMapper().map(
                traceId = TraceId.from(
                    "trace-memory-proposal-result-mapper-005",
                ),
                evaluation = MemoryProposalEvaluationResult.create(
                    traceId = TraceId.from(
    ...
    private fun createPlan(
        ...
        return PlanRecord.create(
            planId = PlanId.from(
                "plan-memory-proposal-result-mapper",
            ),
            task = TaskRecord.create(
                taskId = TaskId.from(
                    "task-memory-proposal-result-mapper",
                ),
