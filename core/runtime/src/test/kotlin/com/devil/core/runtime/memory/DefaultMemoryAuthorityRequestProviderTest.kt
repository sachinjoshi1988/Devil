...
class DefaultMemoryAuthorityRequestProviderTest {

    @Test
    fun `provide returns available request for proposable memory proposal`() {
        val traceId = TraceId.from(
            "trace-memory-authority-request-provider-001",
        )
        val proposalRequest = createProposalRequest(traceId)
        val provider: MemoryAuthorityRequestProvider =
            DefaultMemoryAuthorityRequestProvider()

        val result = provider.provide(
            proposal = MemoryProposalResult.create(
                traceId = traceId,
    ...
    @Test
    fun `provide returns unavailable for deferred memory proposal`() {
        val traceId = TraceId.from(
            "trace-memory-authority-request-provider-002",
        )

        val result =
            DefaultMemoryAuthorityRequestProvider().provide(
                proposal = MemoryProposalResult.create(
                    traceId = traceId,
    ...
    @Test
    fun `provide preserves failed memory proposal error`() {
        val traceId = TraceId.from(
            "trace-memory-authority-request-provider-003",
        )
    ...
    @Test
    fun `provide does not treat proposable result as committed memory`() {
        val traceId = TraceId.from(
            "trace-memory-authority-request-provider-004",
        )
        ...
        val result =
            DefaultMemoryAuthorityRequestProvider().provide(
                proposal = MemoryProposalResult.create(
                    traceId = traceId,
                    status = MemoryProposalStatus.PROPOSABLE,
                    request = createProposalRequest(traceId),
                ),
            )
    ...
    private fun createProposalRequest(
        traceId: TraceId,
    ...
    private fun createPlan(
        ...
        val task = TaskRecord.create(
            taskId = TaskId.from(
                "task-memory-authority-request-provider",
            ),
            decision = decision,
        ...
        return PlanRecord.create(
            planId = PlanId.from(
                "plan-memory-authority-request-provider",
            ),
            task = task,
