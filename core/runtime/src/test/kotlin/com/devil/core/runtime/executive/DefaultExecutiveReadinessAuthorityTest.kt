...
class DefaultExecutiveReadinessAuthorityTest {
    ...
    @Test
    fun `evaluate defers without inventing readiness`() {
        val context = createContext("trace-executive-default-001")
        val authority: ExecutiveReadinessAuthority =
        ...
        val result = authority.evaluate(
            ...
            capability = createCapability(context.traceId),
        )
    ...
    fun `evaluate rejects identity result from a different trace`() {
        val context = createContext("trace-executive-default-002")
        val authority: ExecutiveReadinessAuthority =
            DefaultExecutiveReadinessAuthority()
        ...
        assertFailsWith<IllegalArgumentException> {
            authority.evaluate(
                ...
                identity = createIdentity(
                    TraceId.from("trace-executive-identity-other"),
                ),
                trust = createTrust(context.traceId),
                authorization = createAuthorization(context.traceId),
                understanding = createUnderstanding(context.traceId),
    ...
    @Test
    fun `evaluate rejects trust result from a different trace`() {
        val context = createContext("trace-executive-default-003")
        val authority: ExecutiveReadinessAuthority =
            DefaultExecutiveReadinessAuthority()
        ...
        assertFailsWith<IllegalArgumentException> {
            authority.evaluate(
                ...
                identity = createIdentity(context.traceId),
                trust = createTrust(
                    TraceId.from("trace-executive-trust-other"),
                ),
                authorization = createAuthorization(context.traceId),
                understanding = createUnderstanding(context.traceId),
                decision = createDecision(context.traceId),
    ...
    @Test
    fun `evaluate rejects authorization result from a different trace`() {
        val context = createContext("trace-executive-default-004")
        val authority: ExecutiveReadinessAuthority =
            DefaultExecutiveReadinessAuthority()
        ...
        assertFailsWith<IllegalArgumentException> {
            authority.evaluate(
                ...
                authorization = createAuthorization(
                    TraceId.from("trace-executive-authorization-other"),
                ),
                understanding = createUnderstanding(context.traceId),
                decision = createDecision(context.traceId),
                task = createTask(context.traceId),
    ...
    @Test
    fun `evaluate rejects understanding result from a different trace`() {
        val context = createContext("trace-executive-default-005")
        val authority: ExecutiveReadinessAuthority =
            DefaultExecutiveReadinessAuthority()
        ...
        assertFailsWith<IllegalArgumentException> {
            authority.evaluate(
                ...
                understanding = createUnderstanding(
                    TraceId.from("trace-executive-understanding-other"),
                ),
                decision = createDecision(context.traceId),
                task = createTask(context.traceId),
                plan = createPlan(context.traceId),
    ...
    @Test
    fun `evaluate rejects decision result from a different trace`() {
        val context = createContext("trace-executive-default-006")
        val authority: ExecutiveReadinessAuthority =
            DefaultExecutiveReadinessAuthority()
        ...
        assertFailsWith<IllegalArgumentException> {
            authority.evaluate(
                ...
                decision = createDecision(
                    TraceId.from("trace-executive-decision-other"),
                ),
                task = createTask(context.traceId),
                plan = createPlan(context.traceId),
                capability = createCapability(context.traceId),
    ...
    @Test
    fun `evaluate rejects task result from a different trace`() {
        val context = createContext("trace-executive-default-007")
        val authority: ExecutiveReadinessAuthority =
            DefaultExecutiveReadinessAuthority()
        ...
        assertFailsWith<IllegalArgumentException> {
            authority.evaluate(
                ...
                decision = createDecision(context.traceId),
                task = createTask(
                    TraceId.from("trace-executive-task-other"),
                ),
                plan = createPlan(context.traceId),
                capability = createCapability(context.traceId),
            )
    ...
    fun `evaluate rejects plan result from a different trace`() {
        val context = createContext("trace-executive-default-008")
        val authority: ExecutiveReadinessAuthority =
            DefaultExecutiveReadinessAuthority()
        ...
        assertFailsWith<IllegalArgumentException> {
            authority.evaluate(
                ...
                task = createTask(context.traceId),
                plan = createPlan(
                    TraceId.from("trace-executive-plan-other"),
                ),
                capability = createCapability(context.traceId),
            )
        }
    ...
    fun `evaluate rejects capability result from a different trace`() {
        val context = createContext("trace-executive-default-009")
        val authority: ExecutiveReadinessAuthority =
            DefaultExecutiveReadinessAuthority()
        ...
        assertFailsWith<IllegalArgumentException> {
            authority.evaluate(
                ...
                capability = createCapability(
                    TraceId.from("trace-executive-capability-other"),
                ),
            )
        }
    }
