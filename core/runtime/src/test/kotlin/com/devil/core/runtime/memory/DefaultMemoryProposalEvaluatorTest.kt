...
class DefaultMemoryProposalEvaluatorTest {

    @Test
    fun `evaluate returns unavailable without creating memory proposal`() {
        val traceId = TraceId.from(
            "trace-default-memory-proposal-evaluator-001",
        )
        val evaluator: MemoryProposalEvaluator =
    ...
    @Test
    fun `evaluate does not treat learnability as committed memory`() {
        val traceId = TraceId.from(
            "trace-default-memory-proposal-evaluator-002",
        )
    ...
    fun `evaluate rejects request from another trace`() {
        assertFailsWith<IllegalArgumentException> {
            DefaultMemoryProposalEvaluator().evaluate(
    ...
    private fun createPlan(
        ...
        return PlanRecord.create(
            planId = PlanId.from(
                "plan-default-memory-proposal-evaluator",
            ),
            task = TaskRecord.create(
                taskId = TaskId.from(
                    "task-default-memory-proposal-evaluator",
                ),
