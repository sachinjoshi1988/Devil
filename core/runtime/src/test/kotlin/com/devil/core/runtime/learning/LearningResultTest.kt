...
class LearningResultTest {
    ...
    fun `create preserves learnable result with matching request`() {
        val traceId = TraceId.from(
            "trace-learning-result-001",
        )
        val request = createRequest(traceId)

        val result = LearningResult.create(
            traceId = traceId,
    ...
    fun `create rejects learnable request from a different trace`() {
        assertFailsWith<IllegalArgumentException> {
            LearningResult.create(
                traceId = TraceId.from(
                    "trace-learning-result-005",
                ),
                status = LearningStatus.LEARNABLE,
                request = createRequest(
                    TraceId.from(
                        "trace-learning-result-request-other",
                    ),
                ),
            )
        }
    ...
    fun `create rejects failed error from a different trace`() {
        assertFailsWith<IllegalArgumentException> {
            LearningResult.create(
                traceId = TraceId.from(
                    "trace-learning-result-008",
                ),
                status = LearningStatus.FAILED,
                error = createError(
                    TraceId.from(
                        "trace-learning-result-error-other",
                    ),
                ),
            )
        }
    ...
    private fun createPlan(
        ...
        return PlanRecord.create(
            planId = PlanId.from(
                "plan-learning-result",
            ),
            task = TaskRecord.create(
                taskId = TaskId.from(
                    "task-learning-result",
                ),
