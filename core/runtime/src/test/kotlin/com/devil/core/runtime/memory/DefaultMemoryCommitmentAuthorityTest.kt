...
class DefaultMemoryCommitmentAuthorityTest {

    @Test
    fun `evaluate commitment defers when persistence evidence is unavailable`() {
        val context = createContext(
            "trace-memory-commitment-authority-001",
        )

        val result = evaluateCommitment(
            authority = DefaultMemoryCommitmentAuthority(),
            context = context,
        )
    ...
    @Test
    fun `evaluate commitment coordinates request evaluator and mapper`() {
        val context = createContext(
            "trace-memory-commitment-authority-002",
        )
        val authority = DefaultMemoryCommitmentAuthority(
            evaluator =
                object : MemoryCommitmentEvaluator {
        ...
        assertEquals(
            "capability-memory-commitment-authority",
            result.request
                ?.authorityRequest
                ?.proposal
                ?.learning
    ...
    @Test
    fun `evaluate commitment defers when request is unavailable`() {
        val context = createContext(
            "trace-memory-commitment-authority-003",
        )
        val authority = DefaultMemoryCommitmentAuthority(
            requestProvider =
                object : MemoryCommitmentRequestProvider {
                    override fun provide(
                        memory: MemoryAuthorityResult,
                    ): MemoryCommitmentRequestResult {
                        return MemoryCommitmentRequestResult.create(
    ...
    @Test
    fun `evaluate commitment preserves failed request error`() {
        val context = createContext(
            "trace-memory-commitment-authority-004",
        )
        val error = createError(
            traceId = context.traceId,
            code = "MEMORY_COMMITMENT_REQUEST_FAILED",
        )
        val authority = DefaultMemoryCommitmentAuthority(
            requestProvider =
                object : MemoryCommitmentRequestProvider {
    ...
    @Test
    fun `evaluate commitment preserves failed evaluation error`() {
        val context = createContext(
            "trace-memory-commitment-authority-005",
        )
        val error = createError(
            traceId = context.traceId,
            code = "MEMORY_COMMITMENT_EVALUATION_FAILED",
        )
        val authority = DefaultMemoryCommitmentAuthority(
            evaluator =
                object : MemoryCommitmentEvaluator {
    ...
    @Test
    fun `evaluate commitment rejects Memory Authority result from another trace`() {
        val context = createContext(
            "trace-memory-commitment-authority-006",
        )

        assertFailsWith<IllegalArgumentException> {
            DefaultMemoryCommitmentAuthority().evaluateCommitment(
                context = context,
                ...
                memory = MemoryAuthorityResult.create(
                    traceId = TraceId.from(
                        "trace-memory-commitment-memory-other",
                    ),
    ...
    private fun evaluateCommitment(
        authority: MemoryCommitmentAuthority,
        context: ContextEnvelope,
    ): MemoryCommitmentResult {
        return authority.evaluateCommitment(
            context = context,
            identity = createIdentity(context.traceId),
            trust = createTrust(context.traceId),
            authorization = createAuthorization(context.traceId),
            ...
            learning = createLearning(context),
            memoryProposal = createMemoryProposal(context),
            memory = createMemoryAuthorityResult(context),
        )
    }
    ...
    private fun createTask(
        ...
        return TaskAuthorityResult.create(
            ...
            task = TaskRecord.create(
                taskId = TaskId.from(
                    "task-memory-commitment-authority",
                ),
                decision =
                    requireNotNull(
    ...
    private fun createPlanResult(
        ...
        return PlanAuthorityResult.create(
            ...
            plan = PlanRecord.create(
                planId = PlanId.from(
                    "plan-memory-commitment-authority",
                ),
                task =
                    requireNotNull(
    ...
    private fun createCapability(): CapabilityContract {
        return CapabilityContract.create(
            ...
            name = "Memory Commitment Authority",
            description =
                "Represents one bounded authority test capability without persistence.",
        )
    }
    ...
    private fun createMemoryAuthorityResult(
        ...
        return MemoryAuthorityResult.create(
            ...
            request = MemoryAuthorityRequest.create(
                proposal =
                    requireNotNull(
                        createMemoryProposal(context).request,
                    ),
            ),
        )
    }
    ...
    private fun createError(
        ...
        return UniversalErrorRecord.create(
            ...
            summary =
                "Bounded constitutional memory commitment dependency failed.",
        )
    }
