...
class HomeworkAssistanceStage147Test {

    @Test
    fun `record preserves exact child privacy boundary and normalized assistance context`() {
        val childPrivacyBoundary = childPrivacyBoundary()
        ...
        val record =
            HomeworkAssistanceRecord.create(
                ...
                assistanceApproach =
                    "  Use guided explanation and hints.  ",
            )

        assertSame(
            childPrivacyBoundary,
            record.childPrivacyBoundary,
        )
        assertEquals(
            "Fractions",
            record.homeworkSubject,
        )
        assertEquals(
            "Help the learner understand equivalent fractions.",
    ...
    @Test
    fun `record rejects blank homework subject`() {
    ...
    private fun childPrivacyBoundary(): ChildPrivacyBoundaryRecord {
        val subjectIdentity =
            IdentityId.from("identity:stage147-model-child")
                    ...
                    EducationSessionRecord.create(
                        sessionId =
                            EducationSessionId.from(
                                "education-session:stage147-model",
                            ),
                        subjectIdentityId = subjectIdentity,
                        objective =
