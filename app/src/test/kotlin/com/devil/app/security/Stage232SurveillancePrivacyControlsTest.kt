...
class Stage232SurveillancePrivacyControlsTest {

    @Test
    fun `retained Stage 231 evidence produces controlled privacy context with exact provenance`() {
        val retention =
            retainedEvidence()
            ...
            AndroidSurveillancePrivacyControlsCoordinator()
                .apply(
                    ...
                    privacyControlsDescription =
                        "  Preserve bounded surveillance privacy controls.  ",
                )
        ...
        assertSame(
            retention.ownerSecurityDashboard,
            result.evidenceRetention.ownerSecurityDashboard,
        )
        assertEquals(
            "Preserve bounded surveillance privacy controls.",
            result.privacyControlsDescription,
        )
    }

    @Test
    fun `blank privacy controls description keeps Stage 232 deferred`() {
        val retention =
            retainedEvidence()
    ...
    @Test
    fun `deferred Stage 231 evidence keeps Stage 232 deferred`() {
        val retention =
            deferredEvidenceRetention()
        ...
        val result =
            AndroidSurveillancePrivacyControlsCoordinator()
                .apply(
                    evidenceRetention = retention,
                    privacyControlsDescription =
                        "Bounded surveillance privacy controls.",
                )
    ...
    @Test
    fun `controlled result requires retained Stage 231 evidence`() {
    ...
    @Test
    fun `controlled result preserves exact Stage 231 provenance`() {
        val retention =
            retainedEvidence()
    ...
    @Test
    fun `controlled result rejects blank privacy controls description`() {
        assertFailsWith<IllegalArgumentException> {
            AndroidSurveillancePrivacyControlsResult.create(
                status =
                    AndroidSurveillancePrivacyControlsStatus.CONTROLLED,
    ...
    @Test
    fun `deferred result cannot smuggle privacy controls metadata`() {
        assertFailsWith<IllegalArgumentException> {
            AndroidSurveillancePrivacyControlsResult.create(
                status =
                    AndroidSurveillancePrivacyControlsStatus.DEFERRED,
