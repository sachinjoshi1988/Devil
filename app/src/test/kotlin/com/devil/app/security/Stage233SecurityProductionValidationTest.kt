...
class Stage233SecurityProductionValidationTest {

    @Test
    fun `controlled Stage 232 privacy context produces validated security production context with exact provenance`() {
        val privacyControls =
            controlledPrivacy()
        ...
        val result =
            AndroidSecurityProductionValidationCoordinator()
                .prepare(
                    ...
                    validationFocus =
                        "  Phase O bounded security production readiness  ",
                    validationEvidenceDescription =
                        "  Stages 224 through 232 preserve bounded security and surveillance contracts.  ",
                )
        ...
        assertSame(
            privacyControls,
            result.privacyControls,
        )
        assertSame(
            privacyControls.evidenceRetention,
            result.privacyControls?.evidenceRetention,
        )
        assertEquals(
            "Phase O bounded security production readiness",
    ...
    @Test
    fun `deferred Stage 232 privacy context keeps Stage 233 deferred`() {
        val privacyControls =
            AndroidSurveillancePrivacyControlsResult.create(
                status =
    ...
    @Test
    fun `blank validation focus keeps Stage 233 deferred`() {
        val result =
            AndroidSecurityProductionValidationCoordinator()
                .prepare(
    ...
    @Test
    fun `blank validation evidence keeps Stage 233 deferred`() {
        val result =
            AndroidSecurityProductionValidationCoordinator()
                .prepare(
                    privacyControls = controlledPrivacy(),
    ...
    @Test
    fun `validated result requires controlled Stage 232 privacy context`() {
        val deferredPrivacy =
            AndroidSurveillancePrivacyControlsResult.create(
                status =
                    AndroidSurveillancePrivacyControlsStatus.DEFERRED,
        ...
        assertFailsWith<IllegalArgumentException> {
            AndroidSecurityProductionValidationResult.create(
                status =
                    AndroidSecurityProductionValidationStatus.VALIDATED,
                privacyControls = deferredPrivacy,
                validationFocus =
                    "Phase O bounded security production readiness",
                validationEvidenceDescription =
                    "Bounded structural validation evidence.",
    ...
    @Test
    fun `validated result preserves exact Stage 232 provenance`() {
        val privacyControls =
            controlledPrivacy()
        ...
        assertSame(
            privacyControls.evidenceRetention.ownerSecurityDashboard,
            result.privacyControls
                ?.evidenceRetention
    ...
    @Test
    fun `validated result rejects blank validation focus`() {
        assertFailsWith<IllegalArgumentException> {
            AndroidSecurityProductionValidationResult.create(
                status =
                    AndroidSecurityProductionValidationStatus.VALIDATED,
    ...
    @Test
    fun `validated result rejects blank validation evidence`() {
        assertFailsWith<IllegalArgumentException> {
            AndroidSecurityProductionValidationResult.create(
                status =
                    AndroidSecurityProductionValidationStatus.VALIDATED,
    ...
    @Test
    fun `deferred result cannot smuggle Stage 232 or validation metadata`() {
        assertFailsWith<IllegalArgumentException> {
            AndroidSecurityProductionValidationResult.create(
                status =
                    AndroidSecurityProductionValidationStatus.DEFERRED,
    ...
    private fun controlledPrivacy():
        AndroidSurveillancePrivacyControlsResult {
        return AndroidSurveillancePrivacyControlsCoordinator()
            .apply(
                evidenceRetention =
                    retainedEvidence(),
                privacyControlsDescription =
                    "Bounded Stage 233 surveillance privacy controls.",
            )
    }

    private fun retainedEvidence():
        AndroidSecurityEvidenceRetentionResult {
        return AndroidSecurityEvidenceRetentionCoordinator()
            .retain(
                ownerSecurityDashboard =
                    availableDashboard(),
                retentionDescription =
                    "Bounded Stage 232 retained security evidence.",
            )
    }

    private fun deferredEvidenceRetention():
        AndroidSecurityEvidenceRetentionResult {
