...
class Stage226SecurityEventUnderstandingTest {

    @Test
    fun `available Stage 225 adapter produces understood event with exact signal provenance`() {
        val adapter =
            ...
            AndroidSecurityEventUnderstandingCoordinator()
                .integrate(
                    ...
                    understandingDescription =
                        "  Preserve bounded supplied security event context.  ",
                )
        ...
        assertEquals(
            "Preserve bounded supplied security event context.",
            result.understandingDescription,
        )
    }

    @Test
    fun `blank understanding description keeps Stage 226 deferred`() {
        val adapter =
            availableCameraAdapter()
    ...
    @Test
    fun `deferred Stage 225 adapter keeps Stage 226 deferred`() {
        val available =
            availableCameraAdapter()
        ...
        val result =
            AndroidSecurityEventUnderstandingCoordinator()
                .integrate(
                    cameraAdapter = deferred,
                    understandingDescription =
                        "Bounded supplied event context.",
                )
    ...
    @Test
    fun `understood result requires available Stage 225 adapter`() {
        val available =
            availableCameraAdapter()
        ...
        assertFailsWith<IllegalArgumentException> {
            AndroidSecurityEventUnderstandingResult.create(
                status =
                    AndroidSecurityEventUnderstandingStatus.UNDERSTOOD,
                cameraAdapter = deferred,
                signal = signal,
    ...
    @Test
    fun `understood result rejects reconstructed surveillance signal provenance`() {
        val adapter =
            availableCameraAdapter()
    ...
    @Test
    fun `deferred result cannot smuggle signal or understanding metadata`() {
        val adapter =
            availableCameraAdapter()
